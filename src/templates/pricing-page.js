  ...
  return (
    <>
      ...
      <TumblerGrid>
        ...
        >
          <Discount style={{ position: 'absolute', right: 0, top: -18 }}>
            - 40%
          </Discount>
          annual
        </PlanType>
  ...
  return (
    <div style={{ width: '100%' }}>
      <ItemsWrapper>
        {plans[0] &&
          plans[0].map((plan) => (
            <PlanCard key={plan.name} plan={plan} check={check} />
          ))}
      </ItemsWrapper>
    </div>
  )
        ...
        <Tier>
          ...
          {plan.name !== 'CUSTOM' && (
            <PriceContainer>
              <OldPrice
                check={check}
              >{`$ ${plan.pricing.monthly.mon}`}</OldPrice>
              <div>
                <Price>
                  {`$ ${
...
function AllPlansInclude() {
  return (
    <DivAllPlansInclude className="all-plans-include">
