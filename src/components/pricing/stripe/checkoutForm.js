      ...
      <Box margin={{ top: '15px' }} pad={{ bottom: '32px' }} direction='row'>
        <Box>
          ...
          <Box
            ...
            border={{ color: expiryError ? 'red' : focusExpiry ? '#00acff' : '' }}
            round='2px'
            pad='7px 12px'
          >
            <CardExpiryElement
        ...
        <Box style={{ position: 'relative' }} margin={{ left: '15px' }}>
          <CvcTooltip ready={cvcInfo}>
            The 3-digit code on the back of most cards. For American Express, it is the
...
const CheckoutFormModal = (check, plan, closeForm, isSubscribing, subscribe, subscribeResult) => {
  const { email } = useUserInfo();
  const [readyFrom, setIsReadyForm] = useState(false);
  const hist = useHistory();

  return (
    <Layer style={{ backgroundColor: '#ffffff00', borderRadius: 0 }} animation={false}>
      <div
        style={{
          width: '456px',
          height: 'auto',
      ...
      >
        ...
        {subscribeResult === undefined && (
          <PopupContent
            ...
            alignContent='center'
          >
            <PlanHeading>{`${plan.name} ${check ? 'ANNUAL' : 'MONTHLY'}`}</PlanHeading>
            <Email>{email}</Email>
